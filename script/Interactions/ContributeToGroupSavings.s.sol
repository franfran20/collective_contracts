...
contract ContributeToGroup is Script {
    // chain names
    ...
    function run() external {
        contributeToGroup(AMOUNT, GROUP_ID, AVALANCHE_CHAIN_NAME);
    }

    function contributeToGroup(uint256 amount, uint256 groupID, string memory chainName) public {
        address collectiveAddress = getCollectiveAddress(chainName);
        ICollectiveCore collectiveCore = ICollectiveCore(collectiveAddress);

        vm.startBroadcast();
        collectiveCore.contributeToGroup(groupID, amount);
        vm.stopBroadcast();
    }
