...
contract StartSavingsScript is Script {
    // anvil accounts
    ...
    function run() external {
        startSavings(AMOUNT, TIME, REASON, TARGET, AVALANCHE_CHAIN_NAME);
    }
    ...
    function startSavings(
        ...
        if (keccak256(abi.encodePacked(chainName)) == keccak256(abi.encodePacked(AVALANCHE_CHAIN_NAME))) {
            ...
            vm.startBroadcast();
            collectiveCore.startSavings(asset, amount, time, reason, target);
            vm.stopBroadcast();
