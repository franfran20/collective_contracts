...
contract WithdrawContributionScript is Script {
    // chain names
    ...
    function run() external {
        //
        claimGroupContribution(GROUP_ID, AVALANCHE_CHAIN_NAME);
    }

    function claimGroupContribution(uint256 groupID, string memory chainName) public {
        address collectiveAddress = getCollectiveAddress(chainName);
        ICollectiveCore collectiveCore = ICollectiveCore(collectiveAddress);

        vm.startBroadcast();
        collectiveCore.claimGroupContribution(groupID);
        vm.stopBroadcast();
    }
