...
contract WithdrawSavingsScript is Script {
    // chain names
    ...
    function run() external {
        //
        withdrawSavings(AVALANCHE_CHAIN_NAME);
    }

    function withdrawSavings(string memory chainName) public {
        address collectiveAddress = getCollectiveAddress(chainName);
        ICollectiveCore collectiveCore = ICollectiveCore(collectiveAddress);

        vm.startBroadcast();
        collectiveCore.withdrawSavings();
        vm.stopBroadcast();
    }
