...
contract CollectiveCoreOptimism is ICollectiveCore, CCIPReceiver, Ownable {
        ...
        checkIfUserSavingTimeHasExpired(msg.sender)
    {
        ...
        _topUpSavings(msg.sender, amount, s_optimismChainSelector);

        bytes memory innerPayload = abi.encode(amount);
        bytes memory encodedPayload = abi.encode(s_topUpSavingsPath, innerPayload, s_optimismChainSelector, msg.sender);
    ...
    function depositUSDT(uint256 amount) external checkAmountIsntZero(amount) {
        ...
        s_cosmicProvider[msg.sender].oUSDT += amount;
        ...
        bytes memory encodedPayload = abi.encode(s_depositUsdtPath, innerPayload, s_optimismChainSelector, msg.sender);
    ...
    function reedemUSDT(uint256 amount, uint64 selectedChainSelector) external checkAmountIsntZero(amount) {
        ...
        if (s_UsdtBalances.Optimism < amount) {
        ...
        bytes memory encodedPayload = abi.encode(s_reedemUsdtPath, innerPayload, s_optimismChainSelector, msg.sender);
    ...
    function breakSavings() external checkUserCanBreakSave(msg.sender) {
        ...
        (uint256 interestToAddToPool, uint256 avaxBufferAmount, uint256 maticBufferAmount) = _breakSavings(msg.sender);
        uint256 opBufferAmountNotInUse = 0;

        bytes memory innerPayload = abi.encode(
            interestToAddToPool, avaxBufferAmount, opBufferAmountNotInUse, maticBufferAmount, s_totalExpectedSaveTime
        );
        bytes memory encodedPayload = abi.encode(s_breakSavingsPath, innerPayload, s_optimismChainSelector, msg.sender);
    ...
    function withdrawSavings() external checkUserCanWithdrawSuccesfully(msg.sender) {
        ...
        uint256 usersChainBalance = s_savingsDetails[msg.sender].savingsBalance.wOP;
        if (usersChainBalance > 0) IERC20(s_wOP).transfer(msg.sender, usersChainBalance);

        s_interestPoolBalance -= usersShareInInterestPool;
        s_UsdtBalances.Avalanche -= usersShareInInterestPool;
        s_totalChainSavings.wOP -= usersChainBalance;
    ...
    function _breakSavings(address user) internal returns (uint256, uint256, uint256) {
        ...
        if (userSavingBalance.wAVAX > 0) {
            ...
            s_interestPoolBalance += avaxBufferAmount;
            s_UsdtBalances.Avalanche += maticBufferAmount;
        }
    ...
    function _handleWithdrawSavingsMessagePath(address saver, bytes memory withdrawSavingsPayload, uint64) internal {
        ...
        uint256 userBalanceOnThisChain = s_savingsDetails[saver].savingsBalance.wOP;
        if (userBalanceOnThisChain > 0) {
            IERC20(s_wOP).transfer(saver, userBalanceOnThisChain);
        }

        s_interestPoolBalance -= usersShareInInterestPool;
        s_UsdtBalances.Optimism -= usersShareInInterestPool;
        s_totalChainSavings.wOP -= userBalanceOnThisChain;
