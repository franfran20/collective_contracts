 ...
 */
contract CollectiveCoreAvalanche is ICollectiveCore, CCIPReceiver {
    ...
    mapping(uint256 groupID => mapping(address user => bool membership)) private s_isMember;
    ...
    /// @notice wrapped avax token and usdt
    ...
    /// @notice optimsim and polygon contract addresses on destination chains
    ...
    /// @notice groupID counter
    /// @dev the groupIDCounter reps the location in the array of group savings - 1
    ...
    /**
     * @notice constructor
     * @param asset Wrapped avax token address
     * @param router The address of the ccip router for avalanche
    ...
    function withdrawSavings() external checkUserCanWithdrawSuccesfully(msg.sender) {
        ...
        if (usersChainBalance.wAVAX > 0) IERC20(s_wAVAX).transfer(msg.sender, usersChainBalance.wAVAX);
        ...
        bytes memory innerPayload =
            abi.encode(s_UsdtBalances.Avalanche, s_interestPoolBalance, s_totalSavers, s_totalExpectedSaveTime);
     ...
     * @inheritdoc ICollectiveCore
     * @notice Includes Modifier Checks For:
     * Zero Amount, Target > Amount, Saving Time > 0
     */
    function createGroupSavings(
        ...
        external
        checkAmountIsntZero(amount)
        IndividualSavingsTargetGreaterThanOrEqualToSavingAmount(targets, amount)
        checkSavingTimeIsNotZero(time)
    {
        IERC20(s_wAVAX).transferFrom(msg.sender, address(this), amount);
        ...
        GroupSavingDetails memory groupSavingsCreated = GroupSavingDetails({
            ...
            creator: msg.sender,
            recipient: recipient,
            savingStartTime: block.timestamp,
            savingStopTime: block.timestamp + time,
            members: 1,
        ...
        s_isMember[groupIDCounter][msg.sender] = true;
        s_contribution[groupIDCounter][msg.sender].wAVAX = amount;

        bytes memory innerPayload =
            abi.encode(groupIDCounter, amount, purpose, recipient, block.timestamp, block.timestamp + time, targets);
        bytes memory encodedPayload =
            abi.encode(s_createGroupSavingsPath, innerPayload, s_avalancheChainSelector, msg.sender);
        ...
        checkGroupSavingTimeIsntOver(groupID)
    {
        ...
        s_contribution[groupID][msg.sender].wAVAX += amount;

        bytes memory innerPayload = abi.encode(groupID, amount);
        bytes memory encodedPayload =
            abi.encode(s_contributeToGroupSavingsPath, innerPayload, s_avalancheChainSelector, msg.sender);
        ...
        checkCanDispatchGroupFunds(groupID)
    {
        ...
        uint256 savedAmountOnThisChain = groupSavingDetails[groupID - 1].amountRaised.wAVAX;
        address recipient = groupSavingDetails[groupID - 1].recipient;

        s_dispatched[groupID] = true;

        if (savedAmountOnThisChain > 0) {
            IERC20(s_wAVAX).transfer(recipient, savedAmountOnThisChain);
        }

        bytes memory innerPayload = abi.encode(groupID);
        bytes memory encodedPayload =
            abi.encode(s_dispatchGroupFundsSavingsPath, innerPayload, s_avalancheChainSelector, msg.sender);
        ...
        checkUserCanClaimContribution(groupID)
    {
        ...
        uint256 userContributionOnThisChain = s_contribution[groupID][msg.sender].wAVAX;
        if (userContributionOnThisChain > 0) {
            uint256 amountToTransferToUser = (userContributionOnThisChain * (100 - AVALANCHE_DEFAULT_FEE)) / 100;
            uint256 protocolProfit = (userContributionOnThisChain * AVALANCHE_DEFAULT_FEE) / 100;

            IERC20(s_wAVAX).transfer(msg.sender, amountToTransferToUser);
        ...
        bytes memory innerPayload = abi.encode(groupID);
        bytes memory encodedPayload =
            abi.encode(s_claimGroupContributionMessagePath, innerPayload, s_avalancheChainSelector, msg.sender);
    ...
    function _ccipReceive(Client.Any2EVMMessage memory receivedMessage) internal override {
        ...
        // No need for handling create group savings here because its always and only called from avalanche.
    ...
    function _handleWithdrawSavingsMessagePath(
        ...
        if (sourceChainSelector == s_polygonChainSelector) {
            s_UsdtBalances.Polygon = updatedUsdtBalancesFromSourceChain;
        }

        CrossChainAssets memory userBalances = s_savingsDetails[saver].savingsBalance;
        if (userBalances.wAVAX > 0) {
            IERC20(s_wAVAX).transfer(saver, userBalances.wAVAX);
        }
    ...
    /**
     * @notice handles decoding parameters and executing the contribute group savings function from a source chain
    ...
    /**
     * @notice handles decoding parameters and executing the dispatch group savings function from a source chain
    ...
    function _handleClaimGroupContributionMessagePath(
        ...
        uint256 userContributionOnThisChain = s_contribution[groupID][claimer].wAVAX;
        if (userContributionOnThisChain > 0) {
            uint256 amountToTransferToUser = (userContributionOnThisChain * (100 - AVALANCHE_DEFAULT_FEE)) / 100;
            uint256 protocolProfit = (userContributionOnThisChain * AVALANCHE_DEFAULT_FEE) / 100;
            s_protocolProfit += protocolProfit;

            IERC20(s_wAVAX).transfer(claimer, amountToTransferToUser);
        }
    ...
    function getUserSavingCompletionPercentage(address user) public view returns (uint256) {
        ...
        if (amountSaved.wOP > 0) {
            if (target.wOP > amountSaved.wOP * 100) {
                ...
                opEthPercentage = (amountSaved.wOP * 100) > 100 ? 100 : ((amountSaved.wOP * 100) / target.wOP);
            }
        }
        ...
        if (amountSaved.wMATIC > 0) {
            if (target.wMATIC > amountSaved.wMATIC * 100) {
                ...
                maticPercentage = (amountSaved.wMATIC * 100) > 100 ? 100 : ((amountSaved.wMATIC * 100) / target.wMATIC);
            }
        }
    ...
    modifier IndividualSavingsTargetGreaterThanOrEqualToSavingAmount(uint256[3] memory target, uint256 amount) {
        uint256 avalancheTarget = 0;
        if (target[avalancheTarget] < amount) {
            revert CollectiveCore__TargetAmountEqualsZero(avalancheTarget);
        }
        _;
    }
